...
  event.preventDefault(); 
  ...
  if (form.dataset.submitting === "true") {
    console.warn("Submit em progresso, clique ignorado.");
    return; 
  }
...
    await scheduleNew({ id, name, when: when.toISOString() });
...
    cancelButton.setAttribute("aria-label", `Cancelar agendamento de ${name} às ${time.textContent}`);
    const cancelIcon = document.createElement("img");
    cancelIcon.classList.add("cancel-icon");
    cancelIcon.setAttribute("src", "./src/assets/icons/cancel.svg"); // Ajustei o caminho que você mencionou
...
    console.log(error);
    ...
    // Libera o lock
    setTimeout(() => { form.dataset.submitting = "false"; }, 300);
  }
}

// --- INÍCIO DA CORREÇÃO DEFINITIVA (Controle de Referência) ---
...
if (window.currentSubmitHandler) {
  ...
  console.log("Removendo listener de submit duplicado...");
  form.removeEventListener("submit", window.currentSubmitHandler);
}

// 3. Anexa o NOVO listener
console.log("Anexando listener de submit...");
form.addEventListener("submit", handleFormSubmit);

// 4. GUARDA a referência da função que ACABAMOS de anexar
//    para que possamos removê-la na PRÓXIMA recarga (se houver).
